...
DWORD EntryUtility::FindEmptyEntry(HANDLE hFilesRiver) {
	...
	if(!ReadFile(hFilesRiver, &tFileEntry, sizeof(struct FileEntry), &rlen, NULL)) {
		if(ERROR_HANDLE_EOF != GetLastError()) return INVALID_INDEX_VALUE;
	}

	if(INVALID_INDEX_VALUE != tFileEntry.nsake_bin) {
		lEmptyIndex = tFileEntry.nsake_bin;
		if(INVALID_INDEX_VALUE == ReadNode(hFilesRiver, &tEmptyEntry, tFileEntry.sibling))
			 return INVALID_INDEX_VALUE;

		if(INVALID_INDEX_VALUE != tEmptyEntry.sibling) {
			tFileEntry.nsake_bin = tEmptyEntry.sibling;
			ModifyNode(hFilesRiver, tEmptyEntry.sibling, FILE_INDEX_NSAKE, tEmptyEntry.nsake_bin);
		} else tFileEntry.nsake_bin = tEmptyEntry.nsake_bin;
...
DWORD EntryUtility::AddEmptyEntry(HANDLE hFilesRiver, DWORD lEmptyIndex) {
	struct FileEntry tFileEntry;
	DWORD rlen, wlen;
	/////////////////////////////////////////////////////////////////////////////
	...
	/////////////////////////////////////////////////////////////////////////////
	ModifyNode(hFilesRiver, lEmptyIndex, FILE_INDEX_NSAKE, tFileEntry.nsake_bin);
	/////////////////////////////////////////////////////////////////////////////
	tFileEntry.nsake_bin = lEmptyIndex;
	OVERLAPPED OverLapped = {0, 0, 0, 0, NULL}; // used for file unlocks.
	OverLapped.Offset = lEmptyOffset;
	if(!WriteFile(hFilesRiver, &tFileEntry, sizeof(struct FileEntry), &wlen, &OverLapped))
		return INVALID_INDEX_VALUE;
