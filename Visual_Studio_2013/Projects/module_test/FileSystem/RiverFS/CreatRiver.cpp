...
DWORD CCreatRiver::InitializeHandle(const TCHAR *szLocation){
	...
	CommonUtility::get_name(szFileName, szLocation, RIVER_DIRECTORY_DEFAULT);
...
static BOOL TraverseDirectory (HANDLE, HANDLE, TCHAR *, TCHAR *, LONG);
static DWORD FileType (LPWIN32_FIND_DATA);
static DWORD FileItem (HANDLE, HANDLE, LPWIN32_FIND_DATA, LONG);
static DWORD FolderItem (HANDLE, TCHAR *, LPWIN32_FIND_DATA, LONG);
...
static BOOL TraverseDirectory (HANDLE hFilesRiver, HANDLE hRiverFolder, TCHAR *szPathName, TCHAR *szPathString, LONG lParentIndex) {
	HANDLE SearchHandle;
...
static DWORD FileType (LPWIN32_FIND_DATA pFileData) {
	DWORD nFileType = TYPE_FILE;
	if (pFileData->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
		if (!lstrcmp(pFileData->cFileName, _T (".")) || !lstrcmp(pFileData->cFileName, _T ("..")))
			nFileType = TYPE_DOT;
		else if(!_tcsncmp(pFileData->cFileName, _T ("RECYCLER"), 0x08)) nFileType = TYPE_RECY;
		else nFileType = TYPE_DIR;
...
static DWORD FileItem (HANDLE hRiverFolder, HANDLE hFilesRiver, LPWIN32_FIND_DATA pFileData, LONG lParentIndex) {
...
/* ParName is the parent directory name, terminated with
a \ or the null string. szFileName has no wildcards at this point. */
static DWORD FolderItem (HANDLE hRiverFolder, TCHAR *szPathString, LPWIN32_FIND_DATA pFileData, LONG lParentIndex) {
