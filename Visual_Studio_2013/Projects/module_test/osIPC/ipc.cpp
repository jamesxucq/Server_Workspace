...
#define DONE_WRITE		0x00000001 // write node
#define CLEAR_WRITE		0xfffffff0

#define DONE_FILLED		0x00010000 // fill full
...
osIPC::Server::Server(void) {
	...
	m_hAtomMod = INVALID_HANDLE_VALUE;
	m_pBuf = NULL;
};
...
bool osIPC::Server::create(wchar_t *connectAddr) {
	...
	wchar_t szAddress[IPC_MAX_ADDR], szEvtAvail[IPC_MAX_ADDR], szEvtFilled[IPC_MAX_ADDR], szAtomMod[IPC_MAX_ADDR], szShareMem[IPC_MAX_ADDR];
	...
	_stprintf_s(szAtomMod, IPC_MAX_ADDR, _T("%s_mtex_modify"), szAddress);
...
logger(_T("d:\\osIPC.log"), _T("server_create: failed: %01d\n"), __LINE__);
	...
	// Cretate the Mutex
	m_hAtomMod = CreateMutex(NULL, FALSE, szAtomMod);
	if (NULL == m_hAtomMod || INVALID_HANDLE_VALUE == m_hAtomMod) { 
logger(_T("d:\\osIPC.log"), _T("server_create: failed: %01d\n"), __LINE__);
		return false; 
	}
...
void osIPC::Server::retBlock(Block* pBlock) {
	WaitForSingleObject(m_hAtomMod, INFINITE);
...
bool osIPC::Client::create(wchar_t *connectAddr) {
	// Address of this server
	wchar_t szAddress[MAX_PATH], szEvtAvail[MAX_PATH], szEvtFilled[IPC_MAX_ADDR], szAtomMod[IPC_MAX_ADDR], szShareMem[MAX_PATH];
