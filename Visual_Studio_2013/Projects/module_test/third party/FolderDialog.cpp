...
int CFolderDialog::OnValidateFailed(LPCTSTR lpstrFolderPath) {
	int iResult = AfxMessageBox(_T("The path entered is not valid! Continue ?"), MB_YESNO|MB_ICONQUESTION|MB_DEFBUTTON2);
	// Return 1 = continue, 0 = EndDialog
	return((iResult == IDYES) ? 0 : 1);
}
...
{
	CFolderDialog* pThisDialog = (CFolderDialog*)(lpData);
	pThisDialog->m_hWnd = hWnd;
	int iResult = 0;

	switch(uMsg) {
	case BFFM_INITIALIZED:
		pThisDialog->OnInitialized();
		break;
	case BFFM_SELCHANGED:
		pThisDialog->OnSelChanged((LPITEMIDLIST)lParam);
		break;
	case BFFM_VALIDATEFAILED:
		iResult = pThisDialog->OnValidateFailed((LPCTSTR)lParam);
		break;
	}
	pThisDialog->m_hWnd = NULL;
	return iResult;	
}
