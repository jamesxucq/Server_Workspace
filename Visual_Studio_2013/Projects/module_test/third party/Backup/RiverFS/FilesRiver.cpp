...
#else
#pragma comment(lib, ".\\dbz\\dbz.lib")
#endif
...
DWORD CFilesRiver::EntryRestore(const TCHAR *szFileName, FILETIME *ftCreationTime, DWORD lParentIndex, DWORD lSiblingIndex) {
	...
	if(INVALID_INDEX_VALUE == EntryUtility::ReadNode(m_hFilesRiver, &tFileEntry, lEmptyIndex))
		return INVALID_INDEX_VALUE;
	///////////////////////////////////////////////////////////////////////
	if(!_tcscmp(tFileEntry.szFileName, szFileName) && !memcmp(&tFileEntry.ftCreationTime, ftCreationTime, sizeof(FILETIME))) {
		//////////////////////////
...
DWORD CFilesRiver::FindRecycled(const TCHAR *szFileName, FILETIME *ftCreationTime) {
	...
	//////////////////////////////////////////////////////////////////////////////
	DWORD lFileIndex = tFillEntry.nsake_bin;
	if(INVALID_INDEX_VALUE != EntryUtility::ReadNode(m_hFilesRiver, &tFillEntry, lFileIndex)) {
		if(!memcmp(&tFillEntry.ftCreationTime, ftCreationTime, sizeof(FILETIME)) && 
			!_tcscmp(tFillEntry.szFileName, szFileName)) {
