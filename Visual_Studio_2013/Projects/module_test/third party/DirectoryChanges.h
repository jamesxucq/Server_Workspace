...
public:
	...
	TCHAR *GetFileNameWithPath(IN TCHAR *szFileName) const;//same as GetFileName() only it prefixes the root path into the file name	
protected:
...
private:
	...
	void ProcessChangeNotifications(IN CFileNotifyInformation & notify_info, IN CWatchDirectory * pdi);
	...
	CWatchDirectory *GetWatchDirectory(IN const TCHAR *szDirectoryName, OUT int &refer_index)const;
	CWatchDirectory *GetWatchDirectory(IN CWatchDirectory * pWatchDirectory, OUT int &refer_index)const;
	CWatchDirectory *GetWatchDirectory(IN CDirectoryHandler * pChangeHandler, OUT int & refer_index)const;
