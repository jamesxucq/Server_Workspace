...
int NBdHTab::BuildHTable(hash_table *htab, HANDLE sums_stre, DWORD file_size) {
	DWORD dwReadSize;
...
//
	...
	if(INVALID_SET_FILE_POINTER==dwResult && NO_ERROR!=GetLastError()) 
		return -1;
//
	int complex_tatol = file_size / sizeof(struct complex_chks);
	for (int inde = 0; inde < complex_tatol; inde++) {
...
// _LOG_DEBUG(_T("offset:%u adler32_chk:%u md5_chks:%s"), read_complex->offset, read_complex->adler32_chk, get_md5sum_textT(read_complex->md5_chks));
