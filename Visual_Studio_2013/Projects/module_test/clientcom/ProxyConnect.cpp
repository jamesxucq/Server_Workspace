...
/*
int NProxyConnect::ConnectHttpAuth(SOCKET ProxySocket, char *szDestiServ, int iDestiPort, char *usr, char *psw) {
...
DWORD NProxyConnect::ConnectHttpAuth(SOCKET ProxySocket, char *szDestiServ, int iDestiPort, char *usr, char *psw) {
	int	 CntSR;
	...
	//发送请求 
	sprintf_s( head_buffer, "CONNECT %s:%d HTTP/1.1\r\nHost: %s\r\nProxy-Authorization: Basic %s\r\nUser-Agent: MSIE 8.0\r\nProxy-Connection: keep-alive\r\n\r\n", szDestiServ, iDestiPort, szDestiServ, szBase64Auth); 
...
DWORD NProxyConnect::ConnectHttp(SOCKET ProxySocket, char *szDestiServ, int iDestiPort) {
	int	 CntSR;
	char head_buffer[HEAD_BUFFER_LEN];

	memset(head_buffer, '\0', HEAD_BUFFER_LEN);
	//发送请求 
	sprintf_s( head_buffer, "CONNECT %s:%d HTTP/1.1\r\nHost: %s\r\nUser-Agent: MSIE 8.0\r\nProxy-Connection: keep-alive\r\n\r\n", szDestiServ, iDestiPort, szDestiServ); 
	//ProxySocket.Send(head_buffer, (int)strlen(head_buffer)); 	
	CntSR = send( ProxySocket, head_buffer, (int)strlen(head_buffer), 0);
	if (CntSR == 0)	return -1;// Graceful close
...
//return value:0=success; -1=connect failed;
DWORD NProxyConnect::ConnectSocks4(SOCKET ProxySocket, char *szDestiServ, int iDestiPort) {
	int	 CntSR;
	char head_buffer[HEAD_BUFFER_LEN];
	...
	ProxyReq->IPAddr = inet_addr(szDestiServ); 
...
/*
int NProxyConnect::ConnectSocks5(SOCKET ProxySocket, char *szDestiServ, int iDestiPort, char *usr, char *psw) {
...
//return value:0=success; -1=connect failed; -2=wrong usr and psw
DWORD NProxyConnect::ConnectSocks5(SOCKET ProxySocket, char *szDestiServ, int iDestiPort, char *usr, char *psw) {
	int	 CntSR;
	char head_buffer[HEAD_BUFFER_LEN];
	...
	if(ProxyAns1->Method == 0x2) { 
		char szUserName[256], szPassword[256];
		strcpy_s(szUserName, usr);
		strcpy_s(szPassword, psw);
		int iUserLen = strlen(szUserName); 
		...
		*(pAuthReq->Ulen) = iUserLen; 
		...
		memcpy(pAuthReq->Name, szUserName, iUserLen);

		pAuthReq->PLen = (pAuthReq->Name + iUserLen);
		*pAuthReq->PLen = iPassLen;

		pAuthReq->Pass = (pAuthReq->PLen + 0x01);
		memcpy(pAuthReq->Pass, szPassword, iPassLen); 

		int iLength = 0x03 + iUserLen + iPassLen;
		//ClientSock.Send(head_buffer, 513);
		CntSR = send( ProxySocket, head_buffer, iLength, 0);
		if (CntSR == 0)	return -1;// Graceful close
	...
	ProxyReq2->Rsv = 0x00; 
	ProxyReq2->Atyp = 0x01; 
	unsigned long tmpLong = inet_addr(szDestiServ); 
...
DWORD NDetectProxy::GetIEPorxyServ(TCHAR *szProxyServ, int *iProxyPort)
{
	HKEY hKey = NULL;
	long lRst = 0;
	...
	TCHAR *splittoken;
	TCHAR *pValue = wcstok_s(szProxyServer, _T(":"), &splittoken);
