...
CTNormalSockets::CTNormalSockets(void):
m_nSendTimeslice(0),
...
DWORD CTNormalSockets::Create(struct SocketsArgu *pSocketsArgu) {
...
DWORD CTNormalSockets::SetSendReceiveSpeed(unsigned long ulMaxReceive, unsigned long ulMaxSend) {
...
DWORD CTNormalSockets::Connect(char *szDestiServ, int iDestiPort) {
	...
	clientService.sin_addr.s_addr = inet_addr(szDestiServ);
	...
	if (clientService.sin_addr.s_addr == INADDR_NONE){
		host = gethostbyname(szDestiServ);
		if (host == NULL){
...
DWORD CTNormalSockets::Send(char *szMessage, size_t length) {
...
DWORD CTNormalSockets::Receive(char *szReceive, int length) {
	static DWORD ulTimestamp;
	static unsigned long ulSendTimesCount;	
	//----------------------
	// Declare and initialize variables.
...
DWORD CTNormalSockets::SendReceive(char *szReceive, char *pSendMessage) {
	int	 result = -1;
...
DWORD CTNormalSockets::SafeSend(char *szMessage, size_t length) {
	int	  bytesSent;
...
DWORD CTNormalSockets::SafeReceive(char *szReceive, int length) {
	//----------------------
	// Declare and initialize variables.
	int bytesRecv = 0;
...
DWORD CTNormalSockets::SafeSendReceive(char *szReceive, char *pSendMessage) {
	int	 result = -1;
...
CReplyNormalSockets::CReplyNormalSockets(void) {
}

CReplyNormalSockets::~CReplyNormalSockets(void) {
}

DWORD CReplyNormalSockets::Create(struct ReplySocksArgu *pReplySocksArgu)
{ return 0; }

DWORD CReplyNormalSockets::SetAddress(char *szDestiServ, int iDestiPort) {
	if(!szDestiServ) return -1;
...
DWORD CReplyNormalSockets::Connect(char *szDestiServ, int iDestiPort) {
	if(!szDestiServ) return -1;
...
DWORD CReplyNormalSockets::Close() {
	//----------------------
	// Graceful close to server.
	shutdown(ConnectSocket, SD_SEND);
	closesocket(ConnectSocket);
...
DWORD CReplyNormalSockets::Send(char *szMessage) {
	int	  bytesSent;
...
DWORD CReplyNormalSockets::Receive(char *szReceive) {
	//----------------------
	// Declare and initialize variables.
	int bytesRecv = 0;
...
DWORD CReplyNormalSockets::SendReceive(char *szReceive, char *pSendMessage) {
	int result = -1;
...
DWORD CReplyNormalSockets::SafeSend(char *szMessage) {
	int	  bytesSent;
...
DWORD CReplyNormalSockets::SafeReceive(char *szReceive) {
	//----------------------
	// Declare and initialize variables.
	int bytesRecv = 0;
	char head_buffer[HEAD_BUFFER_LEN];
...
DWORD CReplyNormalSockets::SafeSendReceive(char *szReceive, char *pSendMessage) {
	int result = -1;
