...
class osIPC {
	...
	struct Block {
		...
		volatile ULONG		Next;						// Next block in the single linked list
	};

	// Shared memory buffer that contains everything required to transmit
	// data between the client and server
	struct MemBuff {
		volatile ULONG		m_Avail;				// List of available blocks
		volatile ULONG		m_Filled;					// List of blocks that have been filled with data
		volatile ULONG		m_FillTail;
...
public:
	...
	public:
		...
		// Block functions
		ULONG					getBlock(DWORD dwTimeout = INFINITE);
		void					retBlock(ULONG pBlock);
	...
	public:
		...
		ULONG					getBlock(DWORD dwTimeout = INFINITE);							// Gets a block
		void					postBlock(ULONG pBlock);										// Posts a block to be processed				
		bool					create(wchar_t *connectAddr);
		void					close(void);
		// Functions
