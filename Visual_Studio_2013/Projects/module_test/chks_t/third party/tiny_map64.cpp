...
struct tiny_map64 *map_file64(HANDLE hfile, unsigned __int64 len, int read_size, int blk_size)
{
	...
	if (!(map = (struct tiny_map64 *)malloc(sizeof (struct tiny_map64))))
		_LOG_WARN(_T("map file malloc error\n"));
...
char *map_ptr64(struct tiny_map64 *map, unsigned __int64 offset,int len)
{
	// size_t nread;
	DWORD nread;
	...
	if (wind_size > map->p_size) {
		map->p = (char *)realloc_array(map->p, sizeof(char), wind_size);
		if (!map->p) {
	...
	* from the previous buffer. */
	...
	if (map->p_fd_offset != read_start) {
		...
		DWORD result = SetFilePointer(map->hfile, read_start&0xffffffff, &move_high, FILE_BEGIN);
