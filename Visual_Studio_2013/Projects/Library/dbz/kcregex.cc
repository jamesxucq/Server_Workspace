...
#else
// #include <regex> // disable by james 20130612
...
bool Regex::compile(const std::string& regex, uint32_t opts) {
  ...
  int32_t cflags = std::tr1::regex::ECMAScript;
  if (opts & IGNCASE) cflags |= std::tr1::regex::icase;
  if ((opts & MATCHONLY) || regex.empty()) cflags |= std::tr1::regex::nosubs;
  try {
    core->rbuf = new std::tr1::regex(regex, (std::tr1::regex::flag_type)cflags);
  } catch (...) {
    core->rbuf = NULL;
    return false;
  }
...
bool Regex::match(const std::string& str) {
  ...
  std::tr1::smatch res;
  return std::tr1::regex_search(str, res, *core->rbuf);
#endif
}
...
std::string Regex::replace(const std::string& str, const std::string& alt) {
  ...
  return std::tr1::regex_replace(str, *core->rbuf, alt);
#endif
}
