...
CTrayIcon::CTrayIcon(UINT uID)
{
	...
	menu.LoadMenu(m_nid.uID); //add by james 20101115
}
...
BOOL CTrayIcon::SetIcon(HICON hicon, LPCTSTR lpTip) 
{
	...
	// Use the tip, if any
	if (lpTip)
		wcsncpy_s(m_nid.szTip, lpTip, countof(m_nid.szTip));
	if (m_nid.szTip[0])
	...
	// Do it
	BOOL boolValue = Shell_NotifyIcon(msg, &m_nid);
	if (msg==NIM_DELETE || !boolValue)
...
LRESULT CTrayIcon::OnTrayNotify(WPARAM wID, LPARAM lEvent)
{
	...
	// 
	//CMenu menu; //delete by james 20101115
	//if (!menu.LoadMenu(m_nid.uID)) return 0; //delete by james 20101115
