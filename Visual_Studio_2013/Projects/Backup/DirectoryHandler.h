...
struct ChangeNotification {
	...
	//Notification Data:
	TCHAR *szFileName1;//<-- is the szFileName parameter to On_FileAdded(),On_FileRemoved,On_FileModified(), and is szExistsFileName to On_FileNameChanged(). Is also szDirectoryName to On_ReadDirectoryChangesError(), On_WatchStarted(), and On_WatchStopped()
...
{
public:
	CDelayedNotifier(){  AddReference(); }
	virtual ~CDelayedNotifier(){ Release(); }
...
private:
	virtual void On_FileAdded(const TCHAR *szFileName); 
	virtual void On_FileRemoved(const TCHAR *szFileName);
	virtual void On_FileNameChanged(const TCHAR *szExistsFileName, const TCHAR *szNewFileName);
	virtual void On_FileModified(const TCHAR *szFileName);
...
public:
	CDirectoryHandler * GetChangeHandler()const { return m_pDirectoryHandler; }
	void SetChangeHandler(CDirectoryHandler *pChangeHandler){ m_pDirectoryHandler = pChangeHandler; }
public:
	...
	void On_FileNameChanged(const TCHAR *szExistsFileName, const TCHAR *szNewFileName);
