...
CFileMon::CFileMon(void): m_hDir(INVALID_HANDLE_VALUE), m_hThread(NULL)
{
...
bool CFileMon::Run( LPCTSTR dir, bool bWatchSubtree, DWORD dwNotifyFilter, LPDEALFUNCTION dealfun, LPVOID lParam )
{
    Close();
    m_hDir = CreateFile(
        dir,
...
void CFileMon::Close( DWORD dwMilliseconds )
{
    if( NULL != m_hThread ) {
        m_bRequestStop = true;
...
DWORD WINAPI CFileMon::Routine( LPVOID lParam )
{
    CFileMon& obj = *(CFileMon*)lParam;
