...
static void PrivilegeEnabler() {
	static int invoke_count;
	if(invoke_count++) return;
	//
	LPCTSTR arPrivelegeNames[] = { SE_BACKUP_NAME, SE_RESTORE_NAME, SE_CHANGE_NOTIFY_NAME, NULL };
	for(int inde = 0; NULL != arPrivelegeNames[inde]; ++inde) {
		if( !EnablePrivilege(arPrivelegeNames[inde], TRUE) ) {
...
static bool IsDirectory(const CString & csPath) {
...
,m_evStartStopEvent(FALSE, TRUE) // NOT SIGNALLED, MANUAL RESET
,m_eRunningState( RUNNING_STATE_NOT_SET )
{	
	...
	memset(szExistFileName, '\0', MAX_PATH);
}
...
CDirectoryHandler * CWatchDirectory::SetDirectoryHandler(CDirectoryHandler * pDirectoryHandler) {
	CDirectoryHandler * pExistHandler = m_pDelayedSender->GetChangeHandler(); 
	m_pDelayedSender->SetChangeHandler(pDirectoryHandler); 
	return pExistHandler;
}
...
DWORD CWatchDirectory::StartMonitor(HANDLE hCompPort) {
	...
	m_eRunningState = RUNNING_STATE_START_MONITORING; // set the state member to indicate that the object is to START monitoring the specified directory
	PostQueuedCompletionStatus(hCompPort, sizeof(CWatchDirectory *), (DWORD)this, &m_stOverlapped); // make the thread waiting on GetQueuedCompletionStatus() wake up
	...
	DWORD dwWait = 0;
	do {
		dwWait = WaitForSingleObject(m_evStartStopEvent, 10 * 1000);
...
BOOL CWatchDirectory::SignalShutdown( HANDLE hCompPort ) {
	...
	m_eRunningState = CWatchDirectory::RUNNING_STATE_STOP;
	// put this object in the I/O completion port... GetQueuedCompletionStatus() will return it inside the worker thread.
	bRetVal = PostQueuedCompletionStatus(hCompPort, sizeof(CWatchDirectory *), (DWORD)this, &m_stOverlapped);
	if( !bRetVal ) {
...
BOOL CWatchDirectory::WaitForShutdown() {
	...
	do{
		dwWait	= MsgWaitForMultipleObjects(1, &m_evStartStopEvent.m_hObject, FALSE, 5000, QS_ALLINPUT); // wait five seconds
		switch( dwWait ) {
		case WAIT_OBJECT_0:
	...
	return (BOOL) (WAIT_OBJECT_0 == dwWait || bWMQuitReceived);
}
...
BOOL CFileNotifyInformation::GetNextNotifyInformation() {
	...
	if( m_pCurrentRecord &&	m_pCurrentRecord->NextEntryOffset!=0UL) {
		...
		PFILE_NOTIFY_INFORMATION pExistRecord = m_pCurrentRecord;
		...
		if( (DWORD)((BYTE*)m_pCurrentRecord - m_pBuffer) > m_dwBufferSize ) {
			...
			m_pCurrentRecord = pExistRecord;
		}					
		return (BOOL)(m_pCurrentRecord != pExistRecord);
	}
	return FALSE;
}
...
TCHAR *CFileNotifyInformation::GetFileNameWithPath(TCHAR *szFileName) const {
	if( m_pCurrentRecord ) {
...
DWORD CDirectoryWatcher::WatchDirectory(const TCHAR *szDirectory, DWORD dwNotifyFilter, CDirectoryHandler *pDirectoryHandler) {
	if(!szDirectory || 0==*szDirectory || 0==dwNotifyFilter || NULL==pDirectoryHandler) {
	...
	if(NULL == m_hCompPort) {
		...
		::SetLastError(dwError); // who knows what the last error will be after inde call pWatchDirectory->DeleteSelf(), so set it just to make sure
		return dwError;
	} else {// completion port created/directory associated w/ it successfully
		if( NULL == m_hThread ) {
			// start the thread
			CWinThread * pThread = AfxBeginThread(MonitorDirectoryChanges, this, THREAD_PRIORITY_HIGHEST);
			if( !pThread ) { // couldn't create the thread!
				TRACE(_T("CDirectoryWatcher::WatchDirectory()-- AfxBeginThread failed!\n"));
				pWatchDirectory->DeleteSelf( );
				return (ERROR_SUCCESS == GetLastError())? ERROR_MAX_THRDS_REACHED : GetLastError();
			} else {
		...
		if( NULL != m_hThread ) { // thread is running, 
...
BOOL CDirectoryWatcher::IsWatchingDirectory(const TCHAR *szDirectory) const {
	...
	int inde;
	if( GetWatchDirectory(szDirectory, inde) ) return TRUE;
	return FALSE;
}
...
UINT CDirectoryWatcher::MonitorDirectoryChanges(LPVOID lpvThisWatcher) {
	...
	do {
		...
		if( !GetQueuedCompletionStatus( pThisWatcher->m_hCompPort, &numBytes, (LPDWORD) &pdi, &lpOverlapped, INFINITE) ) {
			if( !pdi || ( pdi && AfxIsValidAddress(pdi, sizeof(CWatchDirectory))) &&  INVALID_HANDLE_VALUE != pdi->m_hDirectory ) {
		...
		if ( pdi ) { // pdi will be null if I call PostQueuedCompletionStatus(m_hCompPort, 0,0,NULL);
			...
			if( bObjectShouldBeOk ) {
				...
				CWatchDirectory::eRunningState Run_State = pdi->m_eRunningState ;
				VERIFY( pdi->UnlockProperties() ); // let another thread back at the properties...
				switch( Run_State ) 
				{
				case CWatchDirectory::RUNNING_STATE_START_MONITORING:
					{
						if(!pdi->m_cBuffer) { // add by james 20120326
							pdi->m_cBuffer = (char *)malloc(READ_DIR_CHANGE_BUFFER_SIZE);
							if(!pdi->m_cBuffer) {
						...
						if( !ReadDirectoryChangesW( pdi->m_hDirectory,
							pdi->m_cBuffer, // <--FILE_NOTIFY_INFORMATION records are put into this buffer
							...
							NULL) ) // no completion routine!
						{
							...
							pdi->m_eRunningState = CWatchDirectory::RUNNING_STATE_NORMAL;
							pdi->m_dwReadDirError = ERROR_SUCCESS;
							if( pdi->GetDelayedSender() )
								pdi->GetDelayedSender()->On_WatchStarted(ERROR_SUCCESS, pdi->m_szDirectoryName );
						}
						pdi->m_evStartStopEvent.SetEvent();// signall that the ReadDirectoryChangesW has been called
				...
				case CWatchDirectory::RUNNING_STATE_STOP:
					{
						...
						if( INVALID_HANDLE_VALUE != pdi->m_hDirectory ) {
							pdi->CloseDirectoryHandle();
							pdi->m_eRunningState = CWatchDirectory::RUNNING_STATE_STOP_STEP2; // back up step...GetQueuedCompletionStatus() will still need to return from the last time that ReadDirectoryChangesW() was called.....
							pdi->GetDelayedSender()->On_WatchStopped( pdi->m_szDirectoryName );
						} else {
							pdi->m_evStartStopEvent.SetEvent(); // set the event that ReadDirectoryChangesW has returned and no further calls to it will be made...
						}
						if( pdi->m_cBuffer ) { // add by james 20120326
							free(pdi->m_cBuffer);
							pdi->m_cBuffer = NULL; 
				...
				case CWatchDirectory::RUNNING_STATE_STOP_STEP2:
					{
						if( INVALID_HANDLE_VALUE == pdi->m_hDirectory )
							pdi->m_evStartStopEvent.SetEvent(); // signal that no further calls to ReadDirectoryChangesW will be made
				...
				case CWatchDirectory::RUNNING_STATE_NORMAL:
					{
						...
						CFileNotifyInformation notify_info( (LPBYTE)pdi->m_cBuffer, READ_DIR_CHANGE_BUFFER_SIZE);
				...
				} // end switch( pdi->m_eRunningState )
...
void CDirectoryWatcher::ProcessChangeNotifications(CFileNotifyInformation & notify_info, CWatchDirectory * pdi) {
	DWORD ulTimestamp;
	...
	TCHAR *toke = szFileName + _tcslen(pdi->m_szDirectoryName);
	do {
		ulTimestamp = GetTickCount(); 
		notify_info.GetFileNameWithPath( toke );
		switch( notify_info.GetAction() ) {
		case FILE_ACTION_ADDED:		// a file was added!
			pDelayedSender->On_FileAdded( szFileName, ulTimestamp ); 
			break;
		case FILE_ACTION_REMOVED:	// a file was removed
			pDelayedSender->On_FileRemoved( szFileName, ulTimestamp ); 
			break;
		case FILE_ACTION_MODIFIED:
			// a file was changed
			pDelayedSender->On_FileModified( szFileName, ulTimestamp ); 
			break;
		case FILE_ACTION_RENAMED_OLD_NAME:
			_tcscpy_s( pdi->szExistFileName, szFileName );
			break;
		case FILE_ACTION_RENAMED_NEW_NAME:
			if(*(pdi->szExistFileName) != '\0')
				pDelayedSender->On_FileNameChanged( pdi->szExistFileName, szFileName, ulTimestamp );
			break;
		default:
...
BOOL CDirectoryWatcher::UnwatchAllDirectories() {
	if( NULL != m_hThread ) {
		...
		int max = m_arDirectoriesToWatch.GetSize();
		for(int inde = 0; inde < max; ++inde) {
			if( NULL != (pWatchDirectory = m_arDirectoriesToWatch[inde]) ) {
				VERIFY( pWatchDirectory->UnwatchDirectory( m_hCompPort ) );

				m_arDirectoriesToWatch.SetAt(inde, NULL)	;
				pWatchDirectory->DeleteSelf();
			}
		}
		m_arDirectoriesToWatch.RemoveAll();
		// kill off the thread
...
BOOL CDirectoryWatcher::UnwatchDirectory(const TCHAR *szDirectory) {
	...
	if( NULL != m_hCompPort ) {
		CSingleLock lock(&m_csWatchDirectory, TRUE);
		int nInde = -1;
		CWatchDirectory * pWatchDirectory = GetWatchDirectory(szDirectory, nInde);
		if( NULL != pWatchDirectory &&	-1 != nInde ) {
			// stop watching this directory
			VERIFY( pWatchDirectory->UnwatchDirectory( m_hCompPort ) );
			// cleanup the object used to watch the directory
			m_arDirectoriesToWatch.SetAt(nInde, NULL);
			pWatchDirectory->DeleteSelf();
			bRetVal = TRUE;
		}
	}
...
The CWatchDirectory::m_pChangeHandler member of objects in the m_arDirectoriesToWatch
...
BOOL CDirectoryWatcher::UnwatchDirectory(CDirectoryHandler * pChangeHandler) {
	...
	while( NULL != (pWatchDirectory = GetWatchDirectory( pChangeHandler, nInde )) ) {
		VERIFY( pWatchDirectory->UnwatchDirectory( m_hCompPort ) );
		nUnwatched++;
		m_arDirectoriesToWatch.SetAt(nInde, NULL);
		pWatchDirectory->DeleteSelf( );	
...
BOOL CDirectoryWatcher::UnwatchDirectoryBecauseOfError(CWatchDirectory * pWatchDirectory) {
	...
	if( pWatchDirectory ) {
		...
		int nInde = -1;
		if( GetWatchDirectory(pWatchDirectory, nInde) == pWatchDirectory ) {
			// we are actually watching this....
			//	Remove this CWatchDirectory object from the list of watched directories.
			m_arDirectoriesToWatch.SetAt(nInde, NULL);// mark the space as free for the next watch...
			//	and delete it...
			pWatchDirectory->DeleteSelf( );
		}
	}
...
int	CDirectoryWatcher::AddToWatchDirectory(CWatchDirectory * pWatchDirectory) {
	...
	// first try to add it to the first empty slot in m_arDirectoriesToWatch
	int max = m_arDirectoriesToWatch.GetSize();
	int inde;
	for(inde = 0; inde < max; ++inde) {
		if( NULL == m_arDirectoriesToWatch[inde] ) {
			m_arDirectoriesToWatch[inde] = pWatchDirectory;
			break;
		}
	}
	if( inde == max ) { // there where no empty slots, add it to the end of the array
		try{
			inde = m_arDirectoriesToWatch.Add( pWatchDirectory );
		} catch(CMemoryException * e) {
			e->ReportError();
			e->Delete();//??? delete this? I thought CMemoryException objects where pre allocated in mfc? -- sample code in msdn does, so will inde
			inde = -1;
		}
	}
...
CWatchDirectory * CDirectoryWatcher::GetWatchDirectory(const TCHAR *szDirectoryName, int &refer_inde)const {
	// can't be watching a directory if it's you don't pass in the name of it...
	if( 0 == *szDirectoryName ) return FALSE;		  //
	CSingleLock lock(const_cast<CCriticalSection*>(&m_csWatchDirectory), TRUE);

	int max = m_arDirectoriesToWatch.GetSize();
	CWatchDirectory * pWatchDirectory = NULL;
	for(int inde = 0; inde < max; ++inde ) {
		if( NULL!=(pWatchDirectory=m_arDirectoriesToWatch[inde]) && 0==_tcsicmp(pWatchDirectory->m_szDirectoryName, szDirectoryName) ) {
			refer_inde = inde;
...
CWatchDirectory * CDirectoryWatcher::GetWatchDirectory(CWatchDirectory * pWatchDirectory, int & refer_inde)const {
	CSingleLock lock( const_cast<CCriticalSection*>(&m_csWatchDirectory), TRUE);

	int max = m_arDirectoriesToWatch.GetSize();
	CWatchDirectory * pdirectory;
	for(int inde = 0; inde < max; ++inde) {
		if( NULL!=(pdirectory=m_arDirectoriesToWatch[inde]) && pdirectory==pWatchDirectory ) {
			refer_inde = inde;
			return pdirectory;
		}
	}
...
CWatchDirectory * CDirectoryWatcher::GetWatchDirectory(CDirectoryHandler * pChangeHandler, int &refer_inde)const {
	CSingleLock lock( const_cast<CCriticalSection*>(&m_csWatchDirectory), TRUE);

	int max = m_arDirectoriesToWatch.GetSize();
	CWatchDirectory * pdirectory;
	for(int inde = 0; inde < max; ++inde) {
		if( NULL!=(pdirectory=m_arDirectoriesToWatch[inde]) && pdirectory->GetDirectoryHandler()==pChangeHandler ) {
			refer_inde = inde;
			return pdirectory;
		}
	}
