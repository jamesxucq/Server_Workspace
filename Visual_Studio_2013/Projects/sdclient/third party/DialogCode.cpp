...
BOOL DialogCode::BrowseDirectories (HWND hwndDlg, TCHAR *lpszTitle, TCHAR *szDireName)
{
	BROWSEINFOW bi;
	LPITEMIDLIST pidl;
	LPMALLOC pMalloc;
	...
	if (SUCCEEDED (SHGetMalloc (&pMalloc))) 
	{
		...
		bi.lParam = (LPARAM)szDireName;

		pidl = SHBrowseForFolderW (&bi);
		if (NULL != pidl) 
		{
			if (SHGetPathFromIDList(pidl, szDireName)) 
			{
				bOK = TRUE;
			}
...
CString DialogCode::BrowseDirectories(HWND hParentHWnd, const CString &csDlgTitle)    
{  
	TCHAR   szDir[MAX_PATH]; 
	BROWSEINFO   brinfo;  
	ITEMIDLIST   *pidl;  
	...
	if (SUCCEEDED (SHGetMalloc (&pMalloc))) 
	{
		brinfo.hwndOwner = hParentHWnd;  
		brinfo.pidlRoot = NULL;  
		brinfo.pszDisplayName = szDir;  
		brinfo.lpszTitle = csDlgTitle;  
...
VOID DialogCode::WaitCursor ()
{
	...
	// if (NULL == hcWait) hcWait = LoadCursor (NULL, IDC_WAIT);
	SetCursor (hcWait);
	// hCursor = hcWait;
}
...
VOID DialogCode::NormalCursor ()
{
	...
	// if (NULL == hcArrow) hcArrow = LoadCursor (NULL, IDC_ARROW);
	SetCursor (hcArrow);
	// hCursor = NULL;
}
...
VOID DialogCode::ArrowWaitCursor ()
{
	...
	// if (NULL == hcArrowWait) hcArrowWait = LoadCursor (NULL, IDC_APPRUNING);
...
VOID DialogCode::HandCursor ()
{
	...
	// if (NULL == hcHand) hcHand = LoadCursor (NULL, IDC_HAND);
	SetCursor (hcHand);
	// hCursor = hcHand;
}
...
BOOL DialogCode::OpenDirectorise(HWND hParentHWnd, const  TCHAR *szDireName)
{
	if((int)ShellExecute(hParentHWnd, _T("open"), szDireName, NULL, NULL, SW_SHOWNORMAL) <= 32)
		return FALSE;
//
	return TRUE;
}