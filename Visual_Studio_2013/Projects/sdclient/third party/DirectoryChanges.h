...
public:
	...
	// CHAR        m_cBuffer[ READ_DIR_CHANGE_BUFFER_SIZE ];// buffer for ReadDirectoryChangesW
	CHAR		*m_cBuffer; // modify by james 20120326
	...
	TCHAR		szExistFileName[MAX_PATH];
	CCriticalSection m_csProperties;
	CEvent		m_evStartStopEvent;
	enum eRunningState {
...
private:
	...
	CWatchDirectory *GetWatchDirectory(const TCHAR *szDirectoryName, int &refer_inde)const;
	CWatchDirectory *GetWatchDirectory(CWatchDirectory * pWatchDirectory, int &refer_inde)const;
	CWatchDirectory *GetWatchDirectory(CDirectoryHandler * pChangeHandler, int & refer_inde)const;

	HANDLE m_hCompPort;	// i/o completion port
	HANDLE m_hThread;	// MonitorDirectoryChanges() thread handle
	DWORD  m_dwThreadID;
	CTypedPtrArray<CPtrArray, CWatchDirectory*> m_arDirectoriesToWatch; // holds info about the directories that we're watching.
