...
struct pmap64 *pmap_file64(FILE *fp, uint64 len, int32 read_size, int32 blk_size) {
    ...
    if (!(map = malloc(sizeof (struct pmap64))))
        printf("map file malloc error\n");
    memset(map, 0, sizeof (struct pmap64));
...
char *pmap_ptr64(struct pmap64 *map, uint64 offset, int32 len) {
    int32 nread;
    uint64 wind_start, read_start;
    int32 wind_size, read_size, read_offset;

    if (len <= 0) {
        printf("invalid len passed to map_ptr: %ld\n", (long) len);
        return NULL;
    }
    ...
    if (wind_size > map->p_size) {
        ...
        if (!map->p) printf("map_ptr newc error\n");
    ...
    if (read_size <= 0) {
        printf("invalid read_size of %ld in map_ptr\n", (long) read_size);
        return NULL;
    }
    if (map->p_fd_offset != read_start) {
        if (fseeko64(map->fp, read_start, SEEK_SET)) {
            printf("fseek return error!\n");
            return NULL;
        }
    ...
    while (read_size > 0) {
        nread = fread(map->p + read_offset, 1, read_size, map->fp);
        if (nread <= 0) {
            if (!map->status) map->status = nread ? errno : ENODATA;
