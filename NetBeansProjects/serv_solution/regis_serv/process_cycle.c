...
//
void worker_process_init(struct regis_config *config) {
    /*
...
//
#define RETRY_START_TIMES       64
#define RETRY_START_DELAY       3 // 3s
pid_t start_worker_process(struct regis_config *config, short alloc_port) {
    ...
    switch (child) {
        ...
        case 0: // child
            ...
            while(RETRY_START_TIMES > retry_times++) {
                if (ERR_FAULT == epoll_server(config, alloc_port)) {
...
int master_process_cycle(struct regis_config *config, short bind_port[], unsigned int tatol) {
...
int single_process_cycle(struct regis_config *config, short bind_port[]) {
    if (ERR_FAULT == epoll_server(config, bind_port[0])) {
        _LOG_WARN("epoll server start failed!");
        return ERR_FAULT;
    }
    //
