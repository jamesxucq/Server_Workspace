...
#define MAX_VERIFY_NUM  256
//

int entry_initial(struct riv_live *plive) {
    char file_name[MAX_PATH * 3];
    //
    RIVER_PATH(file_name, plive->location, RIV_CHKS_DEFAULT);
    plive->fpchks = fopen64(file_name, "rb+");
    if (!plive->fpchks) return 0x01;
    //
    RIVER_PATH(file_name, plive->location, RIV_IDEN_DEFAULT);
    plive->fpiden = fopen64(file_name, "rb+");
    if (!plive->fpiden) return 0x01;
    //
    return 0x00;
}

int entry_final(struct riv_live *plive) {
    if (plive->fpiden) fclose(plive->fpiden);
