...
// #define EPOLL_SERV_PORT 8089 // default port
#define LISTENQ 20
...
#define EVENT_DEL(EPO_FD, EPO_LIST, EVD) { \
    ...
    close(EVD->sockfd); \
    if (!set_levd_list(EPO_LIST, EVD)) \
    { _LOG_WARN("levd_list del fail."); } \
...
_LOG_DEBUG("accept:%d", aifd); \
        do { \
            if (valid_server(sin.sin_addr.s_addr)) \
				levd_list = get_levd_list(EPO_LIST); \
            else levd_list = get_listen_elist(EPO_LIST); \
            if (!levd_list) { \
...
#define INITIAL_RECV_EVENT(EVD, EVDAT) \
    ...
    int command = COP_INVA | CTRL_INVA; \
    enum CACHE_METHOD method = cache_command(&command, (char *)EVDAT->head_buffer, EVDAT->hlen); \
...
int epoll_cache(struct cache_config *config, short alloc_port) {
    // init ssev
    sserve_create(config->cache_address, alloc_port);
    //
