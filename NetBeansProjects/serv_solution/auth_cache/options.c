...
int parse_args(char *xml_config, int argc, char *argv[]) {
    ...
    while ((option = getopt_long(argc, argv, sopt, lopt, NULL)) != -1) {
        switch (option) {
            case 'c': /* The option has 1 argument */
    ...
    if (file_name[0]) {
        if (VALID_FILE_PATH(file_name)) {
            fprintf(stderr, "the file name is error! %s", file_name);
            return EXIT_FAILURE;
        }
    } else {
        CREAT_CONF_PATH(file_name, AUTH_SERV_DEFAULT)
    }
